 ...
 * LOCAL-ONLY VERSION for Wallaby Phenox security
 * Fetches only from local disk, never from cloud
 * Version: 3.0 - Local Only
 */

// Configuration - Local paths only
...
/**
 * Fetch from local path only (no cloud access)
 */
async function fetchLocal(filename) {
    ...
    try {
        ...
        console.error(`Failed to load ${filename} from local path`);
        throw new Error(`Could not load ${filename} - ensure file exists in data/ folder`);
    }
...
async function loadNeuroVueData() {
    try {
        const [epidemiology, portfolio, revenue] = await Promise.all([
            fetchLocal('data.json').catch(e => {
                console.warn('Epidemiology data not available:', e);
                return null;
            }),
            fetchLocal('product-portfolio-data.json').catch(e => {
                console.warn('Portfolio data not available:', e);
                return null;
            }),
            fetchLocal('revenue-data.json').catch(e => {
