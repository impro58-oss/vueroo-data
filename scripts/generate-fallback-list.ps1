...
# Find all scan files
$ScanFiles = Get-ChildItem -Path $ScanDir -Filter "top_50_analysis_*.json" |
    Where-Object { $_.Name -match 'top_50_analysis_(\d{8})_(\d{6})\.json' } |
...
$Fallbacks = $ScanFiles | ForEach-Object {
    $Match = $_.Name -match 'top_50_analysis_(\d{8})_(\d{6})\.json'
    if ($Match) {
        $DateStr = $Matches[1]
        $TimeStr = $Matches[2]
...
git add vueroo-portal/public/crypto/fallback-files.json | Out-Null
